...
async function main(req, res) {
  ...
  if (req.method === 'GET' && reqPath === '/') {
    ...
    // JSON类型转换为数组类型
    const initialDomains = tempDomains.map(domain => "www.".concat(domain.name, ".").concat(domain.suffix));
    ...
    req.on('end', async () => {
      ...
      if (domainParts.length > 2) {
        ...
        const reactAppString = ReactDOMServer.renderToString(React.createElement(DomainInfo, {
