...
async function main(req, res) {
    ...
    if (req.method === 'GET' && reqPath === '/') {
        ...
        templateData = templateData
            .replace('</body>', `<script>window.__INITIAL_DOMAINS__ = ${JSON.stringify(initialDomains)};</script></body>`)
    ...
    async function fuc_end() {
        ...
        if (domainParts.length > 2) {
            ...
            const reactAppString = ReactDOMServer.renderToString(<DomainInfo domainData={jsonString} results={results} />);
            // 替换 HTML 字符串中的相关部分
