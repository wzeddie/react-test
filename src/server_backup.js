...
async function main(req, res) {
    ...
    if (req.method === 'GET' && reqPath === '/') {
        ...
        // 替换逻辑
        data = data.replace(regex1, `var Recentlydomain1_value = '${Recentlydomain1}';`);
        data = data.replace(regex2, `var Recentlydomain2_value = '${Recentlydomain2}';`);
        data = data.replace(regex3, `var Recentlydomain3_value = '${Recentlydomain3}';`);
        // 发送替换后的HTML内容
    ...
    function fuc_end() {
        ...
        if (domainParts.length > 2) {
            ...
            getapi(wwwname, suffix, async (senddata) => {
                ...
                htmlString = htmlString.replace(regexdomain, `<td id="domain">${senddata.domain}</td>`);
                htmlString = htmlString.replace(regexstatus, `<td id="status">${senddata.status}</td>`);
                htmlString = htmlString.replace(regexavailable, `<td id="available">${senddata.available}</td>`);
                htmlString = htmlString.replace(regexcreation_datetime, `<td id="creation_datetime">${senddata.creation_datetime}</td>`);
                htmlString = htmlString.replace(regexexpiry_datetime, ` <td id="expiry_datetime">${senddata.expiry_datetime}</td>`);
                htmlString = htmlString.replace(regexinfo, `<td id="info" style="white-space: pre-wrap;">${senddata.info}</td>`);
                ...
                for (let key in yumingable) {
                    ...
                    <span style="position: relative; z-index: 1;margin: 5px;">
                        ...
                        <span style="z-index: 0;">&nbsp; ${yumingable[key]}</span>
                    </span>
                    `;
                    //匹配的关键字，字符长度51个单位
